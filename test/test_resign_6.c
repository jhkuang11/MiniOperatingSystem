...
void test_resign_6_process_d(PROCESS self, PARAM param)
{
    kprintf("%s\n", self->name);
    if (check_sum != 3)
       test_failed(24);
...
void test_resign_6_process_e(PROCESS self, PARAM param)
{
    kprintf("%s\n", self->name);
    if (check_sum != 1)
       test_failed(23);
...
void test_resign_6_process_f(PROCESS self, PARAM param)
{
    kprintf("%s\n", self->name);
    if (check_sum != 0)
       test_failed(23);
...
{
    test_reset();
    create_process(test_resign_6_process_f, 5, 0, "Test process F");
    kprintf("Created process F\n");
    create_process(test_resign_6_process_e, 5, 0, "Test process E");
    kprintf("Created process E\n");
    create_process(test_resign_6_process_d, 5, 0, "Test process D");

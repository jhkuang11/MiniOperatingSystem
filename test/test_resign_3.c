...
void test_resign_3_process_a(PROCESS self, PARAM param)
{
    kprintf("Process: %s\n\n", self->name);
    kprintf("Param: %d\n", param);
...
void test_resign_3_process_c(PROCESS self, PARAM param)
{
    kprintf("\nProcess: %s\n\n", self->name);
    print_all_processes(kernel_window);
    kprintf("\n");
...
{
    test_reset();
    create_process(test_resign_3_process_a, 5, 42, "Test process A");
    kprintf("Created process A\n");
    create_process(test_resign_3_process_c, 7, 0, "Test process C");

...
void test_resign_4_process_d(PROCESS self, PARAM param)
{
    kprintf("Process: %s\n\n", self->name);
...
void test_resign_4_process_e(PROCESS self, PARAM param)
{
    kprintf("\nProcess: %s\n\n", self->name);
    if (check_sum != 0) 
       test_failed(23);
...
{
    test_reset();
    create_process(test_resign_4_process_e, 5, 0, "Test process E");
    kprintf("Created process E\n");
    create_process(test_resign_4_process_d, 5, 0, "Test process D");
